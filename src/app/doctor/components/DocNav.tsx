...
const DocNav: FC = () => {
  ...
  const isLoggedIn: boolean = Boolean(session);
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        ...
        {/* Desktop Navigation */}
        <div
          className={`hidden md:flex items-center gap-2 p-2 rounded-full ${isDarkMode ? "bg-black/30" : "bg-white/40 backdrop-blur-sm"
            }`}
        >
          {navItems.map((item) =>
            "onClick" in item ? (
              <button
                ...
                className={`px-4 py-2 rounded-full font-medium transition-all duration-300 ${isDarkMode
              ...
              <Link
                key={item.path}
                href={item.path}
                className={`relative px-4 py-2 rounded-full font-medium transition-all duration-300 ${pathname === item.path
                  ? isDarkMode
      ...
      {menuOpen && (
        <div
          className={`absolute top-16 right-4 w-64 p-4 rounded-2xl shadow-lg border backdrop-blur-md ${isDarkMode
          ...
          {navItems.map((item) =>
            "onClick" in item ? (
              <button
                ...
                className={`block w-full text-center px-4 py-3 my-2 rounded-xl font-medium transition-all duration-300 ${isDarkMode
                  ? "hover:bg-red-500/20 text-red-400"
                  : "hover:bg-red-100 text-red-600"
                  }`}
              >
              ...
              <Link
                ...
                className={`block text-center px-4 py-3 my-2 rounded-xl font-medium transition-all duration-300 ${pathname === item.path
                  ? isDarkMode
                    ? "bg-cyan-600/20 text-cyan-400"
