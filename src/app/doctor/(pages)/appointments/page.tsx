...
const DoctorAppointments: FC = () => {
  ...
  const [loadingMeet, setLoadingMeet] = useState<number | null>(null);
  ...
  const fetchAppointments = async (): Promise<void> => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/liveconsult/appointments?doctorId=${user.id}`
      );
      const result: { data?: Appointment[]; error?: string } =
        await res.json();
  ...
  const generateMeet = async (
    ...
    try {
      const res = await fetch(
        `/api/consultation/meet?appointmentId=${id}`
      );
  ...
  return (
    <div
      className={`p-6 space-y-8 transition-colors ${isDarkMode
              ...
              <thead>
                ...
                >
                  {[
                    ...
                    <th key={th} className="p-2 border">
                      {th}
                    </th>
                  ))}
                </tr>
                  ...
                  >
                    ...
                    <td className="p-2 border">
                      <select
                        ...
                        className={`border rounded px-2 py-1 ${isDarkMode
