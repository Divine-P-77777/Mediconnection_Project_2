...
export default function PersonalInfoStep({ form, onNext }) {
  ...
  const onError = (errors) => {
    ...
    if (errorMessages) {
      errorToast(Object.values(errors)[0].message); // Show first error
    } else {
      ...
      <Card className={`p-6 shadow-md transition-all duration-300 ${isDarkMode ? "bg-slate-900 border-slate-800" : "bg-white"}`}>
        ...
        <CardContent className="space-y-5">
          ...
          <Controller
            ...
            render={({ field }) => (
              <input {...field} placeholder="Full Name" className={inputClass.replace("pl-14", "pl-3")} />
            )}
          />
          ...
          <Controller
            ...
            render={({ field }) => (
              <select {...field} className={inputClass.replace("pl-14", "pl-3")}>
          ...
          <div>
            ...
            >
              <span className={!form.watch("dob") ? "text-gray-400" : ""}>
                {form.watch("dob") ? format(new Date(form.watch("dob")), "PPP") : "Choose Date of Birth"}
              </span>
              <span className="text-cyan-500 group-hover:scale-110 transition-transform text-xl">📅</span>
            </button>
            ...
            {formState.errors.dob && (
              <p className="text-red-500 text-sm mt-1">{formState.errors.dob.message}</p>
            )}
          </div>

          <Button type="submit" className="w-full bg-cyan-600 hover:bg-cyan-700 text-white py-1 text-lg font-semibold shadow-lg shadow-cyan-500/20 transition-all active:scale-95">
            Continue to Search
          </Button>
        </CardContent>
      </Card>
